...
- Exercised grouped UI changes: frequency drag preserves ratios, Gain drag
  ...
  selected band.
- Captured the rebuilt JUCE editor at the prototype's exact `752 x 454` logical
  state with six reference bands and the same two-band selection, rendered to a
  `1504 x 908` Retina PNG. The reference screenshot is generated by the editor
  regression itself; production response
