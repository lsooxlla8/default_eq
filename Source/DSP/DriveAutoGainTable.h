...
{
// Offline RMS references generated against default_eq's actual eight
...
inline constexpr std::array<std::array<float, 8>, 8> gains {{
...
{
    const auto& row = gains[(size_t)std::clamp(mode, 0, (int)gains.size() - 1)];
